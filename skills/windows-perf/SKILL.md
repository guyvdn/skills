...
A constant fan with no obvious offender is usually **sustained modest load on a thin
chassis**, not a spike. A dozen idle-but-not-quiet services totalling 10–15% keeps a
28W mobile part warm enough that the fan never gets to stop — and no single row in
Task Manager looks worth killing.
...
# 3. Frequency and power plan
Get-CimInstance Win32_Processor | Select-Object Name, MaxClockSpeed, CurrentClockSpeed
powercfg /getactivescheme
```

Then reduce **sustained background load** — that is the lever, not a single kill.
